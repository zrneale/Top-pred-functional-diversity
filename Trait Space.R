...
Finaldata$dompred <- factor(Finaldata$dompred, levels = c("N","S","G","B"))
...
Abundanceordi$dompred <- factor(Abundanceordi$dompred, levels = c("N","S","G","B"))
Abundanceordi$season <- factor(Abundanceordi$season, levels = c("W","Sp","Su","F"))
...
#Run the function.
##I'm not sure if the Traitaverage df needs to be standardized here or not. I need to talk to Volker about this
...
#Reorder season and dompred variables for plotting
Averageordi2$season <- factor(Averageordi2$season, levels = c("W","Sp","Su","F"))
Averageordi2$dompred <- factor(Averageordi2$dompred, levels = c("N","S","G","B"))
...
#Save
ggsave("Figures/PCOA_loadings.tiff")
...
Season.traitspace <- Averageordi2%>%
  ...
  scale_color_manual(values=cbPalette, labels = c("Invertebrate", "Salamander", "Sunfish", "Bass")) +
  scale_fill_manual(values = cbPalette, guide = NULL) +
